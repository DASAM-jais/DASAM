...
per=[[0.939,0.486],[0.939,0.486]]#对好评做出提升型回应的概率，对差评做出提升型回应的概率，研究变量**********************************#########需要固定√
#默认值0.486，实验取值0.1，0.5，0.9
sp_limit=[0,1]#价格质量上下限
o_limit=[0,0.3]#夸大宣传上下限
pci_limit=[0,0.3]#好评激励上下限
u0=0.3##########################需要固定，根据结果拟合
alpha=0.65#评论偏好的均值#######需要固定√
b=b_weight*np.array([0.11,-0.15,0.17,0,0,-0.23])#评论对潜在消费者的单位影响#################需要固定√
#比例固定，但是大小还没固定
a=a_weight*np.array([1,1.5,-1.5,-1.5])#回应对消费者满意度的影响，内部分别是四类评论#####################需要固定，从论文里找一下比例，然后需要确定量级√

Po_val=-0.2#####################需要固定，根据结果拟合
Po_val_std=0.1#####################需要固定，后面可用于做灵敏度分析，这里取合理值即可√
c_rate=0.15#####################需要固定√
room_number=[17,17]#####################需要固定√
p_reply=[[0.198,0.378],[0.198,0.378]]#提供方做出回应的概率，内部先好评后差评#####################需要固定√
